    ...
    <div class="container-fluid anim shadow-lg" style="background:#343A40;">
      <div class="row anim">
        ...
        >
          ...
          >
            <img
              src="@/static/icon.png" class="img-fluid sm:p-4 pt-2"
            />
          ...
          <div class="text-center p-1" :style="{'background':bgleft2}" style="z-index:100;">
            ...
            <ul class="isaktif sm:pl-4 anim mb-3 text-white animated slideInRight " style="background:#232E30;">
              <li class="bord hover:bg-red-500 text-left sm:pl-3" style="font-size:13px;" @click="$router.push('/driver/admin/user')">&nbsp;&nbsp;User&nbsp;&nbsp;<span class="typcn typcn-user"></span></li>
            </ul>
            <div
              class="bord hover:bg-red-500 rounded-b-lg text-left pl-3 mt-2"
              style="border-top:2px solid black;color:white;font-size:13px;"
              @click="$router.push('/driver/admin')"
            >
              DASHBOARD
            </div>
            <!-- END MENU -->
...
export default {
  ...
  data () {
   ...
   beforeMount() {
      let that = this;
      if(!this.$store.state.users){ //atau tanpa ini
        this.$store.dispatch('celogin',{
          true:true,
          false:'/'
        }) 
      }
